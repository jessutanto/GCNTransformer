...
from tensorflow.keras.layers import Input, Dropout, Dense, Lambda, TimeDistributed
from tensorflow.keras.models import Model
from tensorflow.keras.optimizers import Adam

from model_transformer import GraphConvolution, Time2Vec, AttentionBlock
from utils import *
...
if __name__ == "__main__" :
    ...
    time_embedding = TimeDistributed(Time2Vec(kernel_size = 1))(sequence)
    C = K.concatenate([sequence,time_embedding],-1)
    for _ in range(num_transformer_blocks):
      C= AttentionBlock(head_size=head_size, num_heads = num_heads, ff_dim=ff_dim, dropout=dropout)(C)
    T = Dense(X.shape[1])(C)
    ...
    with open('saved_model/gcn_transformer_loss_{}'.format(args.version),'wb') as gcn_transformer_loss:
        pickle.dump(loss_history, gcn_transformer_loss, protocol=pickle.HIGHEST_PROTOCOL)

    model.save('saved_model/gcn_transformer_model_{}'.format(args.version))
    ...
    plt.ylim([0,6])
    plt.legend()
    plt.savefig('saved_model/plot_gcn_transformer_loss_{}'.format(args.version))